...
export const validator = (v: string) => {
  ...
  for (let i = 0, l = vals.length; i < l; i++) {
    const val = vals[i];
    const [n, d,m] = LAYOUT[i];
    const regs = d.map((c) => Number(c[0]));
    const ci = check(val, regs);
    if (ci == -1) {
      return "ERROR_VALUE :" + `${v} :: ${n} ==> ${m}`;
    }
  }
  return "";
};
...
const DETP = { tag: ElTabPane,isDirectRender:true };
...
export const createElem = (layout: TypeLayout[] = defaultLayout) => {
  ...
  for (let i = 0, l = layout.length; i < l; i++) {
    ...
    const n = layout[i];
    const l = LABEL[n];
    const cls = {tag:"div",prop:n,cls:{tag:ElRadioGroup, prop: "type" ,cls:d.map(([label, config]) => getLineElem(label, config, n))}};
    const tab = { ...DETP, label: l,  cls };
    elems.push(tab);
  }
  //
